...
export function LoginHeroPanel({ headline, subtitle, footer }: LoginHeroPanelProps) {
  return (
    <div className="hidden lg:flex lg:w-[52%] xl:w-[55%] relative overflow-hidden flex-col min-h-screen">
      ...
      <div
        className="pointer-events-none absolute -right-20 -top-20 h-96 w-96 rounded-full bg-yellow-400/12 blur-3xl"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute -bottom-28 left-1/5 h-72 w-72 rounded-full bg-blue-500/15 blur-3xl"
        aria-hidden
      />
      <div
      ...
      <div className="absolute top-8 right-10 xl:right-14 z-20 pointer-events-none">
        <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] backdrop-blur-sm px-3 py-2 ios-shadow-md">
          <PrismLogoMark size={30} idPrefix="login-hero-corner" />
          <span className="text-[10px] font-display font-bold uppercase tracking-[0.14em] text-white/85">
            {APP_NAME}
          </span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-8 xl:px-14 py-8 pointer-events-none">
        <div className="flex flex-col items-center w-full max-w-lg">
          ...
          <div className="mt-10 xl:mt-12 w-full text-center space-y-4">
            <div
              className="h-px w-16 mx-auto bg-gradient-to-r from-transparent via-yellow-400/50 to-transparent"
              aria-hidden
            />
